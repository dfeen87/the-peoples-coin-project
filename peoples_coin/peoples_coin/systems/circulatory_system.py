...
class CirculatorySystem:
    ...
    def _generate_token_id(self, goodwill_action_id: int) -> str:
        ...
        token_id = f"{self.TOKEN_ID_PREFIX}{goodwill_action_id}"
        ...
        try:
            with session.begin():
                ...
                if goodwill_action.status != self.COMPLETED_STATUS:
                    msg = f"GoodwillAction ID {goodwill_action_id} status is '{goodwill_action.status}', expected '{self.COMPLETED_STATUS}'."
                    logger.info(msg)
                    return False, msg

                if goodwill_action.minted_token_id is not None:
                    msg = f"GoodwillAction ID {goodwill_action_id} already minted with token ID {goodwill_action.minted_token_id}."
                    logger.info(msg)
                    return False, msg
                ...
                try:
                    ...
                    logger.info(f"Created UserAccount for new user_id {user_id}.")

                user_account.balance += mint_amount_loves
                goodwill_action.minted_token_id = self._generate_token_id(goodwill_action_id)

            logger.info(f"Minted {mint_amount_loves} Loves for user {user_id}. New balance: {user_account.balance:.4f} Loves.")
            return True, f"Minted {mint_amount_loves} Loves. New balance: {user_account.balance:.4f} Loves."

        except NoResultFound:
            msg = f"GoodwillAction with ID {goodwill_action_id} not found."
            logger.error(msg)
            return False, msg
        except Exception as e:
            logger.error(f"Unexpected error minting tokens for GoodwillAction ID {goodwill_action_id}: {e}", exc_info=True)
            return False, f"Unexpected error during minting: {str(e)}"
